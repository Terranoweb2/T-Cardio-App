...
import { type Medication, useUpdateMedication } from '@/hooks/useMedications';
...
const frequencyOptions = [
  { value: 'ONCE_DAILY', label: 'Une fois/jour', times: 1 },
  { value: 'TWICE_DAILY', label: 'Deux fois/jour', times: 2 },
  { value: 'THREE_DAILY', label: 'Trois fois/jour', times: 3 },
  { value: 'EVERY_OTHER_DAY', label: 'Tous les 2 jours', times: 1 },
  { value: 'WEEKLY', label: 'Hebdomadaire', times: 1 },
  { value: 'AS_NEEDED', label: 'Au besoin', times: 0 },
];

export default function EditMedicationModal({ medication, onClose }: EditMedicationModalProps) {
  const [name, setName] = useState(medication.name);
  const [dosage, setDosage] = useState(medication.dosage || '');
  const [frequency, setFrequency] = useState(medication.frequency);
  const [reminderTimes, setReminderTimes] = useState<string[]>(
    medication.reminderTimes?.length ? medication.reminderTimes : ['08:00']
  );
    ...
    updateMutation.mutate(
      {
        ...
        frequency,
        reminderTimes: isAsNeeded ? [] : reminderTimes,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="glass-card rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-cyan-600 to-teal-600 px-6 py-4 flex items-center justify-between shrink-0">
          <h2 className="text-white text-lg font-semibold">Modifier le medicament</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto dark-scrollbar">
          ...
          <div className="flex items-center justify-between">
            <label className="text-sm text-slate-400">Medicament actif</label>
            <button
              type="button"
